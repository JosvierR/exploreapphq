...
function HeroStat({ value, label, icon }: { value: number; label: TranslationKey; icon: string }) {
  ...
  return (
      <div className="pioneer-hero-stat pixel-stat-card">
      <span className="pioneer-hero-stat__icon pixel-stat-icon" aria-hidden="true">
        {icon}
      </span>
        <strong className="pixel-stat-number">
      ...
      <section className="pioneer-hero" id="top" aria-labelledby="pioneer-hero-title">
        ...
        <div className="container pioneer-hero__grid">
          ...
          <div className="pioneer-hero__copy pioneer-hero__copy--enter">
            ...
            <h1 id="pioneer-hero-title" className="pioneer-hero__title pixel-main-title">
            ...
            <p className="pioneer-hero__lead pixel-lead-text">
            ...
            <div className="pioneer-hero__badges" aria-label={t("pioneer.hero.badges.label")}>
              {HERO_BADGES.map((key, index) => (
                  <span
                      key={key}
                      className={`pioneer-badge pixel-hero-badge${index === 0 ? " pixel-hero-badge--accent" : ""}`}
                  >
                <T k={key} />
              </span>
              ))}
            ...
            <div className="pioneer-hero__actions">
              <LiquidButton asChild className="pioneer-liquid-button pixel-cta-button" size="lg" hoverScale={1.02} tapScale={0.98}>
                <a href="#unirme">
                  <T k="pioneer.hero.cta.primary" />
                </a>
              </LiquidButton>
              <a className="pioneer-secondary-button pixel-secondary-button" href="#retos">
                <T k="pioneer.hero.cta.secondary" />
              </a>
            </div>
            <div className="pioneer-hero__flow pixel-flow" aria-hidden="true">
            <span className="pixel-flow-step">
              <T k="pioneer.hero.flow.video" />
            </span>
              <span className="pioneer-hero__flow-arrow pixel-flow-arrow">→</span>
              <span className="pixel-flow-step">
            ...
            <div className="pioneer-hero__stats" aria-label={t("pioneer.hero.stats.label")}>
              {loading && source !== "api" ? (
                  <p className="pioneer-hero__loading pixel-loading-text">
            ...
            {source === "api" && (
                <p className="pioneer-live-pill pioneer-hero__live pixel-live-pill">
                  <T k="pioneer.leaderboard.live" />
                </p>
            )}
          </div>

          {/* Columna derecha: Visual retro gamer */}
          <div className="pioneer-hero__visual pioneer-hero__visual--enter pixel-visual-frame" aria-label={t("pioneer.hero.visual.label")}>
            {/* Marco de juego alrededor de todo el visual */}
            <div className="pixel-game-frame">
              <div className="pioneer-map-card pixel-map-card">
              ...
              <div className="pioneer-phone-stack pixel-phone-stack">
                <img className="pioneer-phone pioneer-phone--side pixel-phone" src={APP_SCREENS.heroStack.left} alt="" loading="eager" />
                <img
                    className="pioneer-phone pioneer-phone--main pixel-phone pixel-phone--main"
                    src={APP_SCREENS.hero}
                    alt={t("pioneer.hero.phone.alt")}
                    loading="eager"
                    fetchPriority="high"
                />
                <img className="pioneer-phone pioneer-phone--side pixel-phone" src={APP_SCREENS.heroStack.right} alt="" loading="eager" />
              </div>
              <div className="pioneer-float-card pioneer-float-card--video pixel-float-card">
                <span className="pioneer-float-card__icon" aria-hidden="true">▶</span>
                <span><T k="pioneer.hero.float.video" /></span>
              </div>
              <div className="pioneer-float-card pioneer-float-card--route pixel-float-card">
                <span className="pioneer-float-card__icon" aria-hidden="true">↝</span>
                <span><T k="pioneer.hero.float.route" /></span>
              </div>
              <div className="pioneer-float-card pioneer-float-card--badge pixel-float-card">
                <span className="pioneer-float-card__icon" aria-hidden="true">★</span>
                <span><T k="pioneer.hero.float.badge" /></span>
              </div>
            </div>
          </div>
        </div>
